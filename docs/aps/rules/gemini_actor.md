...
Use `text` as the exact content to speak.
...
```json
{
  ...
  "performance": {
    "style_prompt": "Say with physical strain, fear hidden under practical irritation, short and breathy: Just don't die on me now"
  }
}
```
...
<Speaker A>: <exact dialogue>
<Speaker B>: <exact dialogue>
```
...
- Put local emotion in the beat direction.
