...
def compile_resemble_prompts(plan: Dict[str, Any]) -> List[str]:
    ...
    for beat in iter_beats(plan):
        ...
        prompts.append(f'{prefix}. "{beat["text"]}"')
    return prompts
...
def compile_gemini_dialogue_request(plan: Dict[str, Any], scene_id: str, output_path: str) -> Dict[str, Any]:
    ...
    dialogue_beats = [beat for beat in scene.get("beats", []) if beat.get("kind") == "dialogue"]
    speaker_ids = []
    ...
    for beat in dialogue_beats:
        ...
        if tag:
            transcript_lines.append(f"{speaker_names[beat['speaker']]}: [{tag}] {beat['text']}")
        else:
            transcript_lines.append(f"{speaker_names[beat['speaker']]}: {beat['text']}")
